# Ægir - Repository Instructions

Ægir is an ESP32-P4 trimix analyzer firmware project built with ESP-IDF and LVGL. It targets the native 480x800 Guition JC4880P443C_I_W with GT911 touch and an onboard ESP32-C6 Wi-Fi coprocessor.
...
## Validation Rules
- Run `make test` from the repository root after code changes.
- For firmware changes, run `make build` and `make size-check` for the relevant P4 revision.
...
```text
software/firmware/
  main/                            # ESP32-P4 application
  simulator/                       # desktop UI simulator
  tests/                           # host-side C++ tests
  scripts/                         # validation and release tools
  web/                             # browser demo
hardware/
  cad/rev04/                       # current Fusion enclosure and print package
  pcb/analyzer/                    # current main KiCad project
  pcb/usb-input/                   # USB daughterboard
  system-review/                   # engineering acceptance and current evidence
